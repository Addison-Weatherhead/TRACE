...
from GP_VAE.utils import *
from GP_VAE.nn_utils import *
from GP_VAE.gp_kernel import *
...
class DiagonalEncoder(tf.keras.Model):
    ...
    def __call__(self, x):
        # I belive mapped is of shape (batch_size, seq_len, z_size)
        mapped = self.net(x)
        return tfd.MultivariateNormalDiag(
            loc=mapped[..., :self.z_size], # :self.z_size means the first z_size elements of the latent vector (i.e. the first half) will be the means
            scale_diag=tf.nn.softplus(mapped[..., self.z_size:])) # second half will be the standard deviations


class JointEncoder(tf.keras.Model):
    def __init__(self, z_size, hidden_sizes=(64, 64), kernel_size=3, transpose=False, **kwargs):
        ...
        self.z_size = int(z_size)
        self.net = make_cnn(2 * z_size, hidden_sizes, kernel_size)
        self.transpose = transpose

    def __call__(self, x):
        '''___call___ method allows for () notation. e.g:
        >>> my_encoder = JointEncoder(z_size=10)
        >>> my_encoder(train_data)

        '''
        mapped = self.net(x)
        if self.transpose:
            # I believe mapped is of shape (batch_size, seq_len, z_size)
            num_dim = len(x.shape.as_list())
            perm = list(range(num_dim - 2)) + [num_dim - 1, num_dim - 2]
            mapped = tf.transpose(mapped, perm=perm) # Now mapped is of shape (batch_size, z_size, seq_len)
            return tfd.MultivariateNormalDiag(
                loc=mapped[..., :self.z_size, :], # loc is the mean vector
                scale_diag=tf.nn.softplus(mapped[..., self.z_size:, :]))  # scale_diag is the diagonal covariance matrix
        return tfd.MultivariateNormalDiag(
...
class BandedJointEncoder(tf.keras.Model):
    def __init__(self, z_size, hidden_sizes=(64, 64), kernel_size=3, data_type=None, **kwargs):
        ...
        self.z_size = int(z_size)
        self.net = make_cnn(3 * z_size, hidden_sizes, kernel_size)
        self.data_type = data_type

    def __call__(self, x):
        mapped = self.net(x)
        # mapped is of shape (batch_size, seq_length, z_size)
        ...
        # mapped_transposed is now of shape (batch_size, z_size, seq_length)
        ...
        # ~10x times faster on CPU than on GPU
...
class Decoder(tf.keras.Model):
    def __init__(self, output_size, hidden_sizes=(64, 64)):
        ...
        # self.var_net = make_nn(output_size, hidden_sizes)
...
class GaussianDecoder(Decoder):
    ...
    def __init__(self, output_size, hidden_sizes=(64, 64)):
        ...
        self.net = make_nn(2*output_size, hidden_sizes)

    def __call__(self, x):
        out = self.net(x)
        mean = out[...,:self.output_size]
        var = tf.nn.softplus(out[..., self.output_size:])
...
class VAE(tf.keras.Model):
    def __init__(self, latent_dim, data_dim, time_length,
                 ...
                 decoder_sizes=(64, 64), decoder=GaussianDecoder,
    ...
    def _compute_loss(self, x, m_mask=None, x_len=None, return_parts=False):
        assert len(x.shape) == 3, "Input should have shape: [batch_size, time_length, data_dim]"
        x = tf.identity(x)  # in case x is not a Tensor already...
        ...
        if x_len is not None:
            x_len = tf.tile(x_len, [self.M * self.K])  # shape=(M*K*BS, TL, D)
            x_len = tf.cast(x_len, tf.bool)

        pz = self._get_prior(time_length=x.shape[1]) # pz means p(z) which is the prior distribution on z
        qz_x = self.encode(x) # qz_x means q(z|x) which is the estimated distribution on z given x
        z = qz_x.sample()
        px_z = self.decode(z)

        # px_z is our estimated distribution over x. we are computing -log(px_z(x)), i.e. negative log probability we see the original data x under our distribution px_z
        nll = -px_z.log_prob(tf.cast(x, tf.float32))  # shape=(M*K*BS, TL, D)
        # Return only elements of nll that are finite (in theory we should never get infinite values). tf.zeros_like is needed because tf.where is weird in that you *need* 0 or 2 values
        nll = tf.where(tf.math.is_finite(nll), nll, tf.zeros_like(nll))
        if m_mask is not None:
        ...
        if self.K > 1:
            # Trying to make qz_x (i.e. q(z|x)) closer to the prior on z, pz (i.e. p(z)))
            ...
            if m_mask is not None:
                weights = weights / (tf.reduce_sum(tf.cast(m_mask, tf.float32), [1, 2]))
            ...
            kl = self.kl_divergence(qz_x, pz, x_len)  # shape=(M*K*BS, TL or d)
            kl = tf.where(tf.math.is_finite(kl), kl, tf.zeros_like(kl))
            kl = tf.reduce_sum(kl, 1)  # shape=(M*K*BS)

            if m_mask is not None:
                nll = nll / (tf.reduce_sum(tf.cast(m_mask, tf.float32), [1, 2]))
                kl = kl / (tf.reduce_sum(tf.cast(m_mask, tf.float32), [1, 2]))
    ...
    def compute_loss(self, x, m_mask=None, x_len=None, return_parts=False):
        del m_mask
    ...
    def get_trainable_vars(self, time_length=None):
...
class HI_VAE(VAE):
    ...
    def compute_loss(self, x, m_mask=None, x_len=None, return_parts=False):
        return self._compute_loss(x, m_mask=m_mask, x_len=x_len, return_parts=return_parts)
...
class GP_VAE(HI_VAE):
    ...
    def _get_prior(self, time_length=None):
        # Generates prior distribution over z
        ...
        if self.prior is None:
            ...
            self.prior = tfd.MultivariateNormalTriL(loc=tf.zeros([self.latent_dim, self.time_length], dtype=tf.float32),
                                                                  scale_tril=tf.linalg.cholesky(kernel_matrix_tiled))
    ...
    def kl_divergence(self, a, b, x_len=0):
        ...
        with tf.device('/cpu:0'):
            ...
            #         ll = int(x_len[i])
            #         print(a.scale.domain_dimension_tensor().shape)
            #         print(a.scale.shape, a.scale.log_abs_determinant().shape)
            #         print(tf.linalg.diag(a.stddev()).log_abs_determinant().shape)
            #         print('Mean:', b.mean().shape, a.mean().shape)
            #         print('B_inv: ', b_inv_a.shape)
            #         break
            #
            #         # sub_a = tfd.MultivariateNormalDiag(loc=a.mean()[i,:, :ll], scale_diag=a.stddev()[i,:, :ll])
                    # sub_b = tfd.MultivariateNormalDiag(loc=b.mean()[:, :ll], scale_diag=b.stddev()[:, :ll])
                    #
                    # kl_div += (self.pz_scale_log_abs_determinant - sub_a.scale.log_abs_determinant() +
                    #           0.5 * (-tf.cast(sub_a.scale.domain_dimension_tensor(), a.dtype) +
                    #                  squared_frobenius_norm(b_inv_a[:, :ll]) + squared_frobenius_norm(
                    #                     sub_b.scale.solve((sub_b.mean() - sub_a.mean())[..., tf.newaxis]))))
        return kl_div